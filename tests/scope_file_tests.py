...
class ScopeFileTests(unittest.TestCase):
    ...
    def test_scope_check_audit_prints_preflight_and_safe_command(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertIn("findings --latest", output)
            self.assertIn("findings --latest --json", output)
    ...
    def test_scope_check_imports_program_json_and_writes_scope_file(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            self.assertEqual(payload["safe_passive_command"], payload["cli_runbook"][0])
            self.assertIn("--yes", payload["cli_runbook"][0])
            self.assertIn("--no-external-tools", payload["cli_runbook"][0])
            self.assertEqual(payload["cli_runbook"][1], "python cli.py findings --latest")
            self.assertEqual(payload["cli_runbook"][2], "python cli.py findings --latest --json")
